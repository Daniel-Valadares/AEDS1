...
int main()
{
  ...
  y = IO_readdouble("Entrar com um outro valor real: ");
  z = IO_readdouble("Entrar com mais um valor real, diferente do ultimo inserido: ");
  // testar valor
  if (y == z) // testar se y = z
  {
    IO_printf("\n\t%s%s\n", "Erro: Mesmo valor no segundo e terceiro numeros inseridos, ", 
              "favor executar novamente o programa e inserir valores corretos");
  }
  else
  {
    if ((y > x && z < x) || (z > x && y < x)) // testa se x esta entre y e z
    {
      IO_printf("\n\t%s%lf%s%lf%s%lf\n", "O valor ", x, " esta entre o valor ", y, " e o valor ", z);
    }
    else
    {
      IO_printf("\n\t%s%lf%s%lf%s%lf\n", "O valor ", x, " nao esta entre o valor ", y, " e o valor ", z);
    }
  } // end else
  // encerrar
