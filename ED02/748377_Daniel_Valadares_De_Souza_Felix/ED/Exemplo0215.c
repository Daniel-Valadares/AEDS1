...
int main()
{
  ...
  // testar valor
  if (x >= 10 && x <= 25) // testar se x pertece ao intervalo [10:25]
  {
    // mostrar valor pertencente ao intervalo
    IO_printf("\n\t%s%d%s\n", "O valor ", x,
              " pertence ao intervaldo [10:25]");
    if (x >= 20 && x <= 60)
    {
      // mostrar se pertencente ao outro intervalo
      IO_printf("\n\t%s\n", "E tambem pertence ao intervaldo [20:60]");
    }
    else
    {
      // mostrar se nao pertencente ao outro intervalo
      IO_printf("\n\t%s\n", "Mas nao pertence ao intervaldo [20:60]");
    }
  }
  else
  {
    ...
    IO_printf("\n\t%s%d%s\n", "O valor ", x,
              " nao pertence ao intervaldo [10:25]");
    if (x >= 20 && x <= 60)
    {
      // mostrar se pertencente ao outro intervalo
      IO_printf("\n\t%s\n", "Mas pertence ao intervaldo [20:60]");
    }
    else
    {
      // mostrar se nao pertencente ao outro intervalo
