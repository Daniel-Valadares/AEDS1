...
// methodo0314 - contar simbolos em uma palavra
void method0314()
{
    // definir dado
    int x = 0;
    ...
    for (y = 0; y < tamanho; y = y + 1)
    {
        if ('A' <= palavra[y] && palavra[y] <= 'Z' || 'a' <= palavra[y] && palavra[y] <= 'z')
        {
            ...
            // contar repeticoes de minuscula
            if ('a' <= palavra[y] && palavra[y] <= 'z')
            {
                minuscola = minuscola + 1;
            }
            // contar repeticoes de maiscula
            else
            {
    ...
    IO_printf("\nNumero de letras: %d\n", z);
    IO_printf("Letras maiusculas: %d \n", maiuscula);
    IO_printf("Letras minuscolas: %d \n", minuscola);
...
int main()
{
    ...
    do
    {
        ...
        IO_println("Opcoes");
        IO_println(" 0 - parar");
        IO_println(" 1 - contar e mostrar todos simbolos que forem letras, ou maiusculas ou minusculas");
        IO_println("");
