/*
 Guia_02E2 - v0.0. - __ / __ / _____
 Author: _____________________
 Para compilar em uma janela de comandos (terminal):

 No Linux : g++  -o Guia02E2   ./Guia02E2.cpp
 No Windows: g++ -o Guia02E2.exe Guia02E2.cpp
...
void decorateWorld(const char *fileName)
{
   ...
   void BEEPERS () // colocar multiplos beepers
    ;{
        ...
        for ( place = 1; place <= 3; place = place + 1 ) // repeticao ate valor 3
        {
            world->set(5, 5, BEEPER); //por beeper
            if (place<3)
            {
            world->set(4, 4, BEEPER); //por beeper
            world->set(5, 4, BEEPER); //por beeper
            }
            if (place<2)
            {
            world->set(3, 3, BEEPER); //por beeper
...
*/
    void playActions(const char *fileName)
    {
        ...
        comandos = comandos + 1; //contar numero de comandos
        ...
        sprintf(msg, "Comandos = %d", comandos);
        // solicitar a exibicao da mensagem
        show_Text(msg);
    } // end playActions ( )
    void readAndPlay(const char *fileName)
    {
        // definir dados
        int action;
        int comandos = 0;
        ...
        std::ifstream archive(fileName);
        // repetir enquanto houver dados
        archive >> comandos;     // tentar ler a primeira linha
        while (stop<=comandos) // testar se nao encontrado o fim
        {
            // mostrar mais um comando
            IO_print(IO_toString(action));
            delay(stepDelay);
            ...
            execute(action);
            // tentar ler a proxima linha
            stop = stop +1; // definir momento de parar
            archive >> action; // tentar ler a próxima linha
...
int main()
{
    ...
    // executar tarefa 
    robot->readAndPlay("Tarefa02E2.txt");
    world->close();
    // encerrar programa
    getchar();
    return (0);
...
//
// Versao Teste
// E1 01. ( OK ) teste de contagem de comandos
*/