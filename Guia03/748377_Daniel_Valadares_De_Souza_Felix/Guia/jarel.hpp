/*
   jarel  - jarel robot2 definitions.
   ...
   (c) 2010, 2016
   https://git.kpi.fei.tuke.sk/kpi/jarel-the-robot2
 */
...
class World
{
 ...
 ~ World ( );               // destructor
   ...
   int    has_robot2   ( int avenue, int street, char symbol );
   void   show        ( );
   void   reset       ( );
...
/**
   robot2     - robot2 object definition.
 */
class robot2
{
  public:                   // robot2 public container
...
/**
   ref_robot2 - Reference to a robot2 structure.
 */
typedef robot2* ref_robot2;
...
/**
   robot2_now - Current reference to a robot2 structure.
 */
ref_robot2 robot2_now = nullptr;
/**
...
/**
   show_Board - Show basic robot2 board.
...
/**
   has_robot2 - Retrive if there is a specified robot2 at a world slot.
   @return robot2 index, if there is one;
           zero, otherwise
   @param avenue - slot (x)
   @param street - slot (y)
   @param symbol - robot2 badge symbol
 */
int World::has_robot2  ( int avenue, int street, char symbol )
{
...
void World::create ( const char* name )
{
  ...
  {
     strcpy ( this->name, "jarel_robot2_-_v0.1" );
  }
  reset ( );
} // end World::create ( )
       ...
       switch ( object )
       {
          ...
          default:
            if ( ('A' <= object && object <= 'Z') ||
                 ('a' <= object && object <= 'z') )
            {
                x = has_robot2 ( avenue, street, object );
                if ( x >= 0 )
                {
                   show_Error ( "ERROR: robot2 is here." );
                }
                else
                {
                   x = has_slot ( avenue, street );
...
/**
   ...
   @param status    - if robot2 is active or not
 */
...
/**
   beepers - Retrieve number of beepers of current robot2.
   @return number of beepers
 */
int Box::nbeepers ( )
{
...
/**
   debug - Show current robot2 information.
 */
void robot2::debug ( )
{
     delay ( stepDelay );
...
/**
   debug_On  - Set debug to show robot2 information.
 */
void robot2::debugOn ( )
{
  has_debug = true;
...
/**
   debugOff - Reset debug to omit robot2 information.
 */
void robot2::debugOff ( )
{
  has_debug = false;
} // end robot2::debugOff ( )

/**
   checkStatus - Retrieve current robot2 status.
   @return true , if robot2 is active;
           false, otherwise
 */
bool robot2::checkStatus ( )
{
     return ( status );
} // end robot2::checkStatus ( )

/**
   turnOn - Set current robot2 on.
 */
void robot2::turnOn  ( )
{
     status = true;
...
/**
   turnOff - Set current robot2 off.
 */
void robot2::turnOff ( )
{
     status = false;

     show_Error ( "robot2 is inactive." );

     debugOff ( );
} // end robot2::turnOff ( )

/**
   create_robot2     - Set robot2 parameters.
   @param avenue    - slot (x)
   @param street    - slot (y)
   @param direction - in world coordinates
   @param beepers   - initial beeper number
   @param name s    - robot2 identification
 */
void robot2::create ( int avenue   , int street ,
                     int direction, int beepers, 
                     const char* name )
{
  if ( world->areValid ( avenue, street ) &&
...
/**
   set  - Modify robot2 position at world coordinates.
   @param avenue    - slot (x)
   @param street    - slot (y)
 */
void robot2::set ( int x, int y )
{
    int  z;
...
/**
   turnLeft - Turn robot2 to the left of its current direction.
 */
...
/**
   frontIsClear - Test if world slot ahead has free space for robot2.
   @return true , if there is an free space;
           false, otherwise
 */
bool robot2::frontIsClear ( )
{
    int  result = true;
    ...
    return ( result );
} // end robot2::frontIsClear ( )

/**
   leftIsClear - Test if world slot at left has free space for robot2.
   @return true , if there is an free space;
           false, otherwise
 */
bool robot2::leftIsClear ( )
{
    int result = true;
    ...
    return ( result );
} // end robot2::leftIsClear ( )

/**
   rightIsClear - Test if world slot at right has free space for robot2.
   @return true , if there is an free space;
           false, otherwise
 */
bool robot2::rightIsClear ( )
{
    int result = true;
    ...
    return ( result );
} // end robot2::rightIsClear ( )

/**
   facing - Test if current robot2 faces a given direction at world.
   @return true , if robot2 is facing the desired direction;
           false, otherwise
   @param  direction - to be tested
 */
bool robot2::facing ( int direction )
{
    bool result = false;
...
/**
   facingNorth - Test if current robot2 faces North.
   @return true , if robot2 is facing the desired direction;
           false, otherwise
 */
bool robot2::facingNorth ( )
{
     return ( facing ( NORTH ) );
} // end robot2::facingNorth ( )

/**
   facingSouth - Test if current robot2 faces South.
   @return true , if robot2 is facing the desired direction;
           false, otherwise
 */
bool robot2::facingSouth ( )
{
     return ( facing ( SOUTH ) );
} // end robot2::facingSouth ( )

/**
   facingEast - Test if current robot2 faces East.
   @return true , if robot2 is facing the desired direction;
           false, otherwise
 */
bool robot2::facingEast ( )
{
     return ( facing ( EAST ) );
} // end robot2::facingEast ( )

/**
   facingWest - Test if current robot2 faces West.
   @return true , if robot2 is facing the desired direction;
           false, otherwise
 */
bool robot2::facingWest ( )
{
     return ( facing ( WEST ) );
} // end robot2::facingWest ( )
...
/**
   move - Move current robot2 one slot ahead.
 */
void robot2::move ( )
{
    if ( checkStatus ( ) &&
...
/**
   face - Set a new direction to the current robot2.
   @param direction - to be set
 */
void robot2::face ( int direction )
{
     if ( checkStatus ( ) &&
          NORTH <= direction && direction <= WEST )
     {
...
/**
   faceNorth - Set north as the new direction of the current robot2.
 */
void robot2::faceNorth ( )
{
     face ( NORTH );
} // end robot2::faceNorth ( )

/**
   faceSouth - Set south as the new direction of the current robot2.
 */
void robot2::faceSouth ( )
{
     face ( SOUTH );
} // end robot2::faceSouth ( )

/**
   faceEast - Set east   as the new direction of the current robot2.
 */
void robot2::faceEast ( )
{
     face ( EAST );
} // end robot2::faceEast ( )

/**
   faceWest - Set west   as the new direction of the current robot2.
 */
void robot2::faceWest ( )
{
     face ( WEST );
} // end robot2::faceWest ( )

/**
   beepers - Retrive number of beepers in the current robot2 bag.
   @return number of beepers in robot2 bag
 */
int robot2::nbeepers ( )
{
    int result = 0;
...
/**
   beepersInBag - Test if there is any beeper in the current robot2 bag.
   @return true , if there is any beeper in robot2 bag;
           false, otherwise
 */
bool robot2:: beepersInBag ( )
{
    bool result = false;
    ...
    return ( result );
} // end robot2::beepersInBag ( )

/**
   nextToArobot2 - Test if there is another robot2 at current world slot.
   @return true , if there is any robot2 at the same slot;
           false, otherwise
 */
bool robot2::nextToArobot2 ( )
{
    bool result = false;
    char c;
    int  k=0;
...
} // end robot2::pickBeeperFromBox ( )

/**
   putBeeperInBox - Push a beeper into a box at current world slot.
 */
void robot2::putBeeperIntoBox ( ref_Box box )
{
     if ( beepers <= 0 )
     {
        show_Error ( "ERROR: No beeper." );
...
} // end robot2::putBeeperInBox ( )

/**
   xAvenue - Retrieve robot2 location on avenue number.
...
/**
   yStreet - Retrieve robot2 location at street number.
...
/**
   areYouHere - Test robot2 location.
   @return true, if is the right location;
           false, otherwise.
   @param avenue    - slot (x)
   @param street    - slot (y)
 */
bool robot2::areYouHere ( int avenue, int street )
{
 // define local
    bool result = false;
